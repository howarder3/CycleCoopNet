...
class Coop_pix2pix(object):
	...
	def build_model(self):
		...
		# encoding
		mu, sigma = self.generator_encoder(self.input_real_data_A, reuse = False)

		# sampling by re-parameterization technique
		z = mu + sigma * tf.random_normal(tf.shape(mu), 0, 1, dtype=tf.float32)
		self.generated_B_origin = self.generator_decoder(z, reuse = False)
  ...
  #           tf.pow(tf.subtract(tf.reduce_mean(self.input_generated_B, axis=0), tf.reduce_mean(self.input_revised_B, axis=0)), 2))
    ...
    #                               		feed_dict={self.input_generated_B: generated_B_origin, self.input_real_data_A: data_A})
				...
				print("Epoch: [{:4d}] [{:4d}/{:4d}] time: {}, eta: {}, d_loss: {:.4f}, g_loss: {:.4f}, r_loss: {:.4f}"
	...
	def generator_decoder(self, input_image, reuse=False):
		with tf.variable_scope("gen", reuse=reuse):
			...
			# 1st hidden layer
			print("z shape: {}".format(z.shape)) #  (?, 784)
			w0 = tf.get_variable('w0', [z.get_shape()[1], n_hidden], initializer=w_init)
			b0 = tf.get_variable('b0', [n_hidden], initializer=b_init)
			h0 = tf.matmul(z, w0) + b0
			h0 = tf.nn.tanh(h0)

			print("h0 shape: {}".format(h0.shape)) #  (?, 784)

			# gen_decode_layer_1_output = (batch_size, 2, 2, num_filter*8*2)
			gen_decode_layer_1_deconv = gen_decode_conv2d(h0, num_filter*8, name='gen_decode_layer_1_deconv') 
	...
	def generator_encoder(self, input_image, reuse=False):
		with tf.variable_scope("gen", reuse=reuse):
			...
			h1 = relu(gen_encode_layer_8_batchnorm)
			print("h1 shape: {}".format(h1.shape)) #  (?, 784)

			# gen_encode_layer_9_conv = gen_encode_conv2d(leaky_relu(self.gen_encode_layer_8_batchnorm), 2, name='gen_encode_layer_9_conv') 
			...
			print("mean shape: {}".format(mean.shape)) #  (?, 784)
