...
class Coop_pix2pix(object):
	...
	def build_model(self):
		...
		self.B2A_gen_loss = self.cycle_loss_var * self.B2A_cycle_loss + tf.reduce_sum(tf.reduce_mean(1.0 / (2 * self.sigma2 * self.sigma2) * tf.square(self.input_revised_B - self.generated_B), axis=0))
  ...
  #           tf.pow(tf.subtract(tf.reduce_mean(self.input_generated_B, axis=0), tf.reduce_mean(self.input_revised_B, axis=0)), 2))
    ...
    #                               		feed_dict={self.input_generated_B: generated_B, self.input_real_data_A: data_A})
				...
				A2B_generator_loss , _ = sess.run([self.A2B_gen_loss, self.A2B_gen_optim],
                                  		feed_dict={self.input_real_data_A: data_A, self.input_revised_A: revised_A, self.input_recovered_A: recovered_A}) # self.input_revised_B: revised_B,
