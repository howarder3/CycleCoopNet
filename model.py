...
class Coop_pix2pix(object):
	...
	def build_model(self):
		...
		# symbolic langevins
		self.A2B_des_langevin_revision = self.A2B_des_langevin_revision(self.input_generated_A)
		self.B2A_des_langevin_revision = self.B2A_des_langevin_revision(self.input_generated_B)
		...
		self.A2B_gen_loss = tf.reduce_sum(tf.reduce_mean(1.0 / (2 * self.sigma2 * self.sigma2) * tf.square(self.input_revised_A - self.generated_A), axis=0))
		...
		self.B2A_gen_loss = tf.reduce_sum(tf.reduce_mean(1.0 / (2 * self.sigma2 * self.sigma2) * tf.square(self.input_revised_B - self.generated_B), axis=0))
  ...
  #           tf.pow(tf.subtract(tf.reduce_mean(self.input_generated_B, axis=0), tf.reduce_mean(self.input_revised_B, axis=0)), 2))
    ...
    #                               		feed_dict={self.input_generated_B: generated_B, self.input_real_data_A: data_A})
				...
				A2B_generator_loss , _ = sess.run([self.A2B_gen_loss, self.A2B_gen_optim],
                                  		feed_dict={self.input_real_data_A: data_A, self.input_revised_A: revised_A}) # self.input_revised_B: revised_B,
				...
				print("Epoch: [{:4d}] [{:4d}/{:4d}] time: {}, eta: {}, A2B_d_loss: {:.4f}, A2B_g_loss: {:.4f}, A2B_R_loss: {:.4f}, B2A_d_loss: {:.4f}, B2A_g_loss: {:.4f}, B2A_R_loss: {:.4f},"
